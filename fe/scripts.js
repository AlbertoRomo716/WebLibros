...
function displayProductsSearch() {
    document.getElementById("product-cards").style.display = "flex";
    document.getElementById("order").style.display = "none";

    var control = document.getElementById('nombreLibro');
    var nombre = control.value;

    let productsHTML = '';
    const product = productList.find(p => p.name === nombre);
        let buttonHTML = `<button class="button-add" onclick="add(${product.id}, ${product.price})">Agregar</button>`;
